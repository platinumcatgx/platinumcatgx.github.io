...
var _BetterFormatting = class {
  ...
  wrap(editor, textRange, symbolStart, symbolEnd) {
    const changes = [
      {
        from: textRange.from,
        text: symbolStart
  ...
  unwrap(editor, textRange, symbolStart, symbolEnd) {
    const changes = [
      {
        from: textRange.from,
        to: {
  ...
  toggleWrapper(editor, view, symbolStart, symbolEnd) {
    ...
    for (const selection of selections) {
      changes.push(...this.setSelectionWrapState(editor, selection, targetWrapState, symbolStart, symbolEnd));
    }
    const transaction = {
      changes
    };
...
var DirectionalCopy = class {
  ...
  directionalCopy(editor, view, direction) {
    ...
    const textToCopyVertical = editor.getRange({ line: anchor.line, ch: 0 }, { line: head.line, ch: headLength });
    if (direction === Direction.Up) {
      ...
      editor.setSelection({ line: anchor.line + addedLines + 1, ch: anchor.ch }, { line: head.line + addedLines + 1, ch: head.ch });
...
var DirectionalMove = class {
  ...
  directionalMove(editor, view, direction) {
    ...
    if (direction === Direction.Left) {
      ...
      const nextChar = editor.getRange({ line: anchor.line, ch: anchor.ch - 1 }, { line: anchor.line, ch: anchor.ch });
      editor.replaceRange(nextChar, { line: head.line, ch: head.ch - 1 }, { line: head.line, ch: head.ch }, "+tweakMoveLeft");
      editor.replaceRange(selection, { line: anchor.line, ch: anchor.ch - 1 }, { line: head.line, ch: head.ch - 1 }, "+tweakMoveLeft");
      ...
      const nextChar = editor.getRange({ line: head.line, ch: head.ch }, { line: head.line, ch: head.ch + 1 });
      editor.replaceRange(nextChar, { line: anchor.line, ch: anchor.ch }, { line: anchor.line, ch: anchor.ch + 1 }, "+tweakMoveLeft");
      editor.replaceRange(selection, { line: anchor.line, ch: anchor.ch + 1 }, { line: head.line, ch: head.ch + 1 }, "+tweakMoveLeft");
      editor.setSelection({ line: anchor.line, ch: anchor.ch + 1 }, { line: head.line, ch: head.ch + 1 });
    } else {
      this.plugin.debug("Something went wrong...");
...
var SelectionHelper = class {
  ...
  selectWord(editor, view) {
    ...
    const wordStart = (_b = (_a = editor.wordAt(anchor)) == null ? void 0 : _a.from) != null ? _b : anchor;
    const wordEnd = (_d = (_c = editor.wordAt(head)) == null ? void 0 : _c.to) != null ? _d : head;
    editor.setSelection({ line: wordStart.line, ch: wordStart.ch }, { line: wordEnd.line, ch: wordEnd.ch });
    return;
  }
};
...
var ObsidianTweaksSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Debug mode").setDesc("Whether to output debug logging to console").addToggle((toggle) => {
...
var _ToggleHeading = class {
  ...
  toggleHeading(editor, view, heading) {
    ...
    const changes = [];
    const linesToSet = import_lodash2.default.uniq(import_lodash2.default.flatten(selections.map((x) => {
      const range = selectionToRange(x);
      return import_lodash2.default.range(range.from.line, range.to.line + 1);
    })));
...
// pnp:/home/runner/work/ObsidianTweaks/ObsidianTweaks/src/main.ts
var ObsidianTweaksPlugin = class extends import_obsidian2.Plugin {
  onload() {
