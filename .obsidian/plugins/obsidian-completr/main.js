...
var DictionaryProvider = class {
  getSuggestions(context, settings) {
    ...
    const list = ignoreCase ? [(_a = this.wordMap.get(firstChar)) != null ? _a : [], (_b = this.wordMap.get(firstChar.toUpperCase())) != null ? _b : []] : [this.wordMap.get(firstChar)];
...
function filterIntoArray(iterable, predicate) {
  const result = [];
...
var WordListSuggestionProvider = class extends DictionaryProvider {
  ...
  loadFromFiles(vault, settings) {
    return __async(this, null, function* () {
      ...
      if (count > 0)
        new import_obsidian.Notice("Loaded " + count + " words from " + BASE_FOLDER_PATH);
    });
  }
...
function findTagCompletionType(currentLineIndex, currentLine, editor) {
...
var FrontMatterSuggestionProvider = class {
  ...
  getSuggestions(context, settings) {
    ...
    if (currentLine.startsWith("publish:")) {
      ...
      if (completionType === "none")
        return [];
      const { query } = matchWordBackwards(context.editor, context.end, (char) => new RegExp("[" + settings.characterRegex + "/\\-_]").test(char), settings.maxLookBackDistance);
  ...
  loadGlobalTags(cache, files) {
    for (let file of files) {
      this.addTagsFromFile(file, cache.getFileCache(file));
    }
  }
  addTagsFromFile(file, cache) {
  ...
  static isInFrontMatter(editor, pos) {
...
var CompletrSettingsTab = class extends import_obsidian3.PluginSettingTab {
  ...
  reloadWords() {
    return __async(this, null, function* () {
      ...
      yield WordList.loadFromFiles(this.app.vault, this.plugin.settings);
